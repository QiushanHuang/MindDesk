...
| Codex session fails to start | Confirm the local `codex` CLI is installed, available on PATH, and logged in. MindDesk starts an embedded SwiftTerm-backed PTY terminal with a temporary session root, short helper scripts, a bound Agent Review source package, a proposal template, and a `service_tier="fast"` override so older `priority` config values do not block startup. The embedded terminal does not pin a model; edit the command field before using `Run`, or send `/model` after Codex opens. Use `+ Prompt Run` when the current Canvas prompt should be passed as an argument. The black output area is a real terminal view, so Codex TUI redraws, cursor movement, and direct keyboard input behave like Terminal.app. Use Proposal Preview after Codex prints a `minddesk.proposal.envelope`; Preview does not apply changes. You can discard the draft, send a revision request back to Codex, or open the envelope in Proposal Review. |
